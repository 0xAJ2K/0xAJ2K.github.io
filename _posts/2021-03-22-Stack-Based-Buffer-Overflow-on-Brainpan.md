...
Execute the Python script with `python ex.py` and you'll see that the application crashes and Immunity is showing an access violation. EIP has been overwritten with "41414141" which is the hex representation of "AAAA", we have a buffer overflow condition.
...
Copy the bad characters below into the script above the "address" variable. We don't need to include \x00 as this is automatically a bad character.
